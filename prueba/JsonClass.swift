...
class JsonClass: NSObject {
    ...
    let urlBase = "https://appcontactosswift.000webhostapp.com/"//url del servidor remoto
    ...
    let fecha = ""
    //Funcion que recibe el array JSON desde el servidor remoto y lo convierte en array tipo NSArray
                ...
                let task = URLSession.shared.dataTask(with: request) { datos_recibidos, response, error in
                    if error != nil{//detectamos un error y devolvemos el array vacío
                        ...
                        do {
                            ...
                            print("error servidor PHP \(String(describing: String(data: datos_recibidos!, encoding: .utf8))) \(parseError)")
